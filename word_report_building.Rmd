...
if(length(building_list) > 1) {
  for(my_building in building_list) {
    building_df_sum <- df_sum %>% dplyr::filter(building == my_building)
    cat('\n\n# Desk utilisation for building ',
        my_building,
        ' (',
        n_distinct(building_df_sum$survey_device_id),
        ' sensors) \n\n',
        sep = "")
    ...
    if(length(floor_list) > 1) {
      for(my_floor in floor_list) {
        floor_df_sum <- building_df_sum %>% dplyr::filter(floor == my_floor)
        cat('\n\n## Desk utilisation for floor ',
            my_floor,
            ' (',
            n_distinct(floor_df_sum$survey_device_id),
            ' sensors) \n\n',
            sep = "")
    ...
    for(team in team_list) {
      if(!is.na(team)) {
        cat1_df_sum <- building_df_sum %>% dplyr::filter(category_1 == team)
        
        if(length(team_list) > 1) {
          cat('\n\n## Desk utilisation for ',
              team,
              ' (',
              n_distinct(cat1_df_sum$survey_device_id),
              ' sensors) \n\n',
              sep = "")
        ...
        for(cat2 in cat2s) {
          ...
          if(length(cat2s) > 1) {
            cat('\n\n## Desk utilisation for ',
                ...
                cat2,
                ' (',
                n_distinct(cat2_df_sum$survey_device_id),
                ' sensors) \n\n',
                sep = "")
          ...
          for(cat3 in cat3s) {
            ...
            if(length(cat3s) > 1) {
              cat('\n\n## Desk utilisation for ',
                  ...
                  cat3,
                  ' (',
                  n_distinct(cat3_df_sum$survey_device_id),
                  ' sensors) \n\n',
                  sep = "")
