...
get_summarised_data <- function(df) {
  ...
  df %>%
    group_by(date = date(obs_datetime), surveydeviceid, devicetype, category_1, category_2,category_3) %>%
    ...
    ungroup(date, surveydeviceid, devicetype, category_1, category_2,category_3) %>%
    add_is_used() %>%
    add_util_category
